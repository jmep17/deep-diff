...
open_command=("$ROOT/node_modules/.bin/cypress" open --browser "$CYPRESS_BROWSER" --e2e)
if [ "$#" -gt 0 ]; then
  open_command+=("$@")
fi
printf -v open_command_string "%q " "${open_command[@]}"
...
echo "Starting E2E dev server on ${E2E_URL} and opening Cypress with ${CYPRESS_BROWSER}..."
echo "Pick a spec in the Cypress UI to watch tests run in the selected browser."
